...
export default class RedisBinaryDownloadUrl {
  ...
  constructor({ platform, arch, version, os }: RedisBinaryDownloadUrlOpts) {
  ...
  async getDownloadUrl(): Promise<string> {
    ...
    const mirror = resolveConfig('DOWNLOAD_MIRROR') ?? 'https://fastdl.redis.org';
   ...
   * (from: https://www.redis.org/dl/win32)
   */
  async getArchiveNameWin(): Promise<string> {
    let name = `redis-${this.platform}`;
    name += `-${this.arch}`;
   ...
   * (from: https://www.redis.org/dl/osx)
   */
  async getArchiveNameOsx(): Promise<string> {
    ...
    if (isNullOrUndefined(version) || semver.gte(version, '4.2.0')) {
      name = `redis-macos`; // somehow these files are not listed in https://www.redis.org/dl/osx
    }
    name += `-${this.arch}`;
   ...
   * (from: https://www.redis.org/dl/linux)
   */
  async getArchiveNameLinux(): Promise<string> {
    let name = `redis-linux`;
    name += `-${this.arch}`;
  ...
  getLinuxOSVersionString(os: LinuxOS): string {
    if (/ubuntu/i.test(os.dist)) {
      ...
      console.warn('There is no offical build of Redis for ArchLinux!');
    } else if (/alpine/i.test(os.dist)) {
      console.warn('There is no offical build of Redis for Alpine!');
    } else if (/unkown/i.test(os.dist)) {
      // "unkown" is likely to happen if no release file / command could be found
      console.warn(
        'Couldnt parse dist infomation, please report this to https://github.com/nodkz/redis-memory-server/issues'
      );
    } else {
    ...
    console.warn(`Falling back to legacy Redis build!`);
  ...
  getUbuntuVersionString(os: LinuxOS): string {
    ...
    if (os.release === '12.04') {
      ...
      if (this.version && this.version.indexOf('3.') === 0) {
        // For Redis 3.x using 1604 binaries, download distro does not have builds for Ubuntu 1804
        // https://www.redis.org/dl/linux/x86_64-ubuntu1604
        name += '1604';
      } else {
        // See fulllist of versions https://www.redis.org/dl/linux/x86_64-ubuntu1804
        name += '1804';
      }
    } else {
      name += '1404';
  ...
  /**
   * Translate input platform to redis useable platfrom
   * @example
   * darwin -> osx
  ...
  /**
   * Translate input arch to redis useable arch
   * @example
   * x64 -> x86_64
   * @param platform The Platform to translate
   */
  translateArch(arch: string, redisPlatform: string): string {
    if (arch === 'ia32') {
      if (redisPlatform === 'linux') {
        return 'i686';
      } else if (redisPlatform === 'win32') {
        return 'i386';
      }
